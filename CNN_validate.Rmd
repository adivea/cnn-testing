...
# 146 unique mounds (of 166 intersecting) appear in 51 of the 332 unique (of 380) grids

grids_n_mounds <- predicted %>% 
#  st_drop_geometry() %>% 
  group_by(X1) %>% 
  count()
length(unique(grids_n_mounds$X1)) # 51, but one is NA
...
```
Let's look at the missing mounds
```{r}
# The remaining 627 unpredicted mounds are in the remaining

missing <- st_intersection(unpredicted, cnn_grid)
```
...
146 mounds was detected out of 773, making the CNN predict (over 60% probability) ca 20% of mounds documented in the study area. Out of the XX gridcells contained in the study area, 50 contained mounds. 
