...
survey_grids <- st_intersection(survey_ch, cnn_grid) # 192 grid cells with 60%+
plot(survey_grids)
```
...
predicted  #146 unique features among 166 features caught via intersecton
...
```
To summarize the previous chunk, 146 unique mounds (of 166 intersecting) appear in 51 of the 192 "survey" grids that have 60% or higher probability of mounds and are at least partially within the TRAP study area. 
The entire satellite image contains 332 grids with 60%+ probability of mounds

### Missing mounds
Let's explore where else the missing mounds are and what probability has been assigned to the cells that contain them
```{r missing}
# The remaining 627 unpredicted mounds are in the remaining grid cells
cnnall_sp <- st_as_sf(cnn_df, coords = c("x","y"), crs = 32635)
cnnall_grid <- st_make_grid(cnnall_sp, cellsize = 250, what = "polygons") 
cnnall_grid <- st_join(st_sf(cnnall_grid), cnnall_sp)

missing <- st_intersection(unpredicted, cnnall_grid)

# Check for duplicates
?distinct()
duplicated(missing$TRAP_Code)
missing <- missing[!duplicated(missing$TRAP_Code),]

hist(missing$mound_probability, main = "Probability of undetected mounds")
...
missing %>% 
  group_by(X1) %>% 
  tally()
# The missing 620 mounds are contained by 259 grid cells of the satellite image 

missing %>% 
  group_by(X1) %>% 
  count() %>% 
  ggplot()+ geom_histogram(aes(n))+ggtitle("Count of missing mounds per gridcell") + theme_bw()
...
Out of the remaining 259 gridcells, 
...
Perhaps I need to slide the probability upwards and see if higher prob is needed to track the mounds.
...
## Further exploration of 60% prob data - Height
